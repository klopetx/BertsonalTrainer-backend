...
3. **SMART-3 — Daily analytical path (P1).** Implement `Silver -> Spark Batch -> PostgreSQL Gold`, including daily rhyme metrics, originality metrics, rhyme-difficulty metrics, final scoring once approved, and daily/weekly/monthly rankings with reproducible, idempotent reprocessing.
